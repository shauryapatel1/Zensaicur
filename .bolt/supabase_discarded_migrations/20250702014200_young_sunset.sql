/*
  ...
  1. Changes
    - Replace the faulty procedural streak logic with a robust SQL approach
    - Use window functions to properly identify consecutive date sequences
    - Fix the trigger syntax to use EXECUTE PROCEDURE instead of EXECUTE FUNCTION
    - Update the fix_all_user_streaks function to use the new logic
  
  2. Reason
    - Previous streak calculation was not correctly identifying consecutive days
    - Window functions provide a more reliable way to group consecutive dates
    - This approach is the standard pattern for solving consecutive sequence problems
*/
...
-- Note: Using EXECUTE PROCEDURE instead of EXECUTE FUNCTION for compatibility
CREATE TRIGGER on_journal_entry_insert
AFTER INSERT ON public.journal_entries
FOR EACH ROW
EXECUTE PROCEDURE public.recalculate_streaks_for_user(NEW.user_id);
