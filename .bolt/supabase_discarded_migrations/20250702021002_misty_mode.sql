/*
  # Fix Streak Calculation Logic Definitively

  1. Changes
    - Replaces the faulty procedural streak logic with a robust, set-based SQL approach
    - Uses window functions to correctly identify consecutive day sequences
    - Fixes the current_streak and best_streak calculations
    - Ensures proper trigger execution for new journal entries
  
  2. Reason
    - Previous implementations had issues with ambiguous column references
    - The procedural approach was error-prone and complex
    - Window functions provide a more reliable and performant solution for this problem
*/
...
DROP FUNCTION IF EXISTS public.trigger_recalculate_streaks();
...
BEGIN
    ...
    date_diffs AS (
        ...
        SELECT
            entry_date,
            entry_date - LAG(entry_date, 1, entry_date - INTERVAL '1 day') OVER (ORDER BY entry_date DESC) AS diff
    ...
    streak_groups AS (
        ...
        SELECT
            entry_date,
            SUM(CASE WHEN diff != -1 THEN 1 ELSE 0 END) OVER (ORDER BY entry_date DESC) AS streak_group
        FROM date_diffs
    ),
    streaks AS (
    ...
    SELECT
        ...
        CASE
            ...
            THEN (SELECT streak_length FROM streaks WHERE streak_group = 0)
            ELSE 0
        END,
...
-- Step 4: Create the new trigger function that calls the recalculate function
...
FOR EACH ROW
EXECUTE FUNCTION public.trigger_recalculate_streaks();
...
-- Create a Node.js script to fix streaks for all users
...
BEGIN
  ...
  SELECT 
    ...
    p.best_streak,
    p.last_entry_date,
    COUNT(j.id)::INT AS total_entries
  FROM 
    profiles p
  LEFT JOIN
    journal_entries j ON p.user_id = j.user_id
  GROUP BY
    p.user_id, p.current_streak, p.best_streak, p.last_entry_date
  ORDER BY
...
GRANT EXECUTE ON FUNCTION public.get_all_user_streaks() TO authenticated;