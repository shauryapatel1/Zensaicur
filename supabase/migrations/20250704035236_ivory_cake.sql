...
BEGIN
    ...
    -- If no entries left, reset streak and update badges accordingly
    IF v_total_entries = 0 THEN
        ...
        SET
            ...
            earned_at = NULL,
            updated_at = NOW()
        WHERE user_id = v_user_id AND badge_id != 'premium-supporter';
    ...
    SET
        current_streak = v_calculated_current_streak,
        best_streak = GREATEST(profiles.best_streak, v_calculated_best_streak),
        last_entry_date = (SELECT MAX(created_at::DATE) FROM public.journal_entries WHERE user_id = v_user_id),
        updated_at = NOW()
    WHERE profiles.user_id = v_user_id;
    ...
    FOR v_badge_record IN SELECT * FROM public.badges LOOP
        ...
        BEGIN
            ...
            -- Update the user_badges record for the current badge
            UPDATE public.user_badges
            SET
                progress_current = v_progress_current,
                progress_percentage = calculated_percentage,
                earned = (v_progress_current >= v_badge_record.progress_target),
                earned_at = CASE 
                    WHEN v_progress_current >= v_badge_record.progress_target AND earned_at IS NULL THEN NOW() 
                    WHEN v_progress_current < v_badge_record.progress_target THEN NULL
                    ELSE earned_at 
                END,
                updated_at = NOW()
            WHERE user_id = v_user_id AND badge_id = v_badge_record.id;
        END;
    END LOOP;
...
-- This version doesn't try to call the trigger function directly
...
BEGIN
    ...
    -- Robust streak calculation
    WITH user_entry_dates AS (
        SELECT DISTINCT created_at::DATE AS entry_date FROM public.journal_entries WHERE user_id = p_user_id
    ), date_groups AS (
        SELECT entry_date, (entry_date - (ROW_NUMBER() OVER (ORDER BY entry_date DESC))::int) AS streak_group FROM user_entry_dates
    ), streaks AS (
        SELECT COUNT(*) AS streak_length, MAX(entry_date) AS last_day FROM date_groups GROUP BY streak_group
...
-- Step 11: Refresh badge progress for all users (without using the trigger function directly)
DO $$
DECLARE
    user_record RECORD;
BEGIN
    FOR user_record IN SELECT user_id FROM public.profiles LOOP
        -- Call the standalone refresh function for each user
