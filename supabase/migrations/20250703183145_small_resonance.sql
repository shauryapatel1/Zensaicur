/*
  ...
  1. Changes
    - Fixed variable naming conflicts in PL/pgSQL functions
    - Renamed variable `current_date` to `entry_date` to avoid conflict with PostgreSQL's built-in CURRENT_DATE function
    - Added proper type casting for EXTRACT results
    - Added missing trigger for journal entry deletion
    - Added function to manually refresh user badge progress
  
  2. Reason
    - Previous migration failed due to syntax error with variable naming
    - Improved reliability of database functions for streak calculation and badge updates
*/
...
-- Recreate recalculate_streaks_for_user function with proper variable naming
...
BEGIN
    ...
    -- Calculate days since first entry with proper type casting
...
-- Create a function to get user badge progress
...
BEGIN
  ...
  FROM 
    badges b
  LEFT JOIN 
    user_badges ub ON b.badge_id = ub.badge_id AND ub.user_id = target_user_id
  ORDER BY 
    b.badge_category, 
    b.progress_target;
END;
