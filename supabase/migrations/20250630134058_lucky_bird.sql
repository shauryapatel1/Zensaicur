...
BEGIN
  ...
  -- Get current streak, best streak, and user creation date
  SELECT 
    profiles.current_streak, 
    profiles.best_streak,
    profiles.subscription_status = 'premium',
    profiles.created_at INTO current_streak, user_best_streak, is_premium, user_created_at
  FROM profiles
  WHERE profiles.user_id = target_user_id;
  
  -- Calculate if user is in trial period (7 days from creation)
  IF user_created_at IS NOT NULL THEN
    days_since_creation := EXTRACT(DAY FROM (NOW() - user_created_at));
    is_in_trial := days_since_creation <= 7;
  ELSE
    is_in_trial := false;
  END IF;
  ...
  RAISE LOG 'User stats - entries: %, streak: %, best: %, moods: %, premium: %, trial: %', 
    entry_count, current_streak, user_best_streak, mood_variety, is_premium, is_in_trial;
    ...
    BEGIN
      ...
      ELSIF badge_record.badge_category = 'special' THEN
        ...
        IF badge_record.id = 'premium-supporter' THEN
          -- Premium supporter badge - earned if user has premium subscription
          UPDATE user_badges
          SET 
...
-- Create a function to check if a user is in trial period
...
BEGIN
  ...
  -- Check if user is within trial period (7 days from creation)
  RETURN (
