/*
  ...
  1. Changes
    - Add SECURITY DEFINER to all database functions
    - Add debug logging with RAISE LOG statements
    - Create a new function to manually refresh badge progress
    - Fix badge progress percentage calculations
    - Update database policies to allow functions to modify badge data

  2. Security
    - Enable functions to run with elevated permissions
    - Ensure proper access control for badge updates
*/
...
BEGIN
  ...
  -- Using cursor-based approach instead of FOR loop
  FOR badge_id, progress_target IN 
    SELECT id, progress_target 
    FROM badges 
    WHERE badge_category = 'entries' AND id != 'first-entry'
  LOOP
    -- Calculate progress percentage
    progress_pct := LEAST(100, ROUND((total_entries::NUMERIC / progress_target::NUMERIC) * 100));
    
    RAISE LOG 'Processing entry badge: %, target: %, current: %, percentage: %', 
      badge_id, progress_target, total_entries, progress_pct;
    ...
    VALUES (
      ...
      total_entries,
      progress_pct,
      total_entries >= progress_target,
      CASE WHEN total_entries >= progress_target THEN NOW() ELSE NULL END
    )
    ON CONFLICT (user_id, badge_id) 
    DO UPDATE SET 
      progress_current = total_entries,
      progress_percentage = progress_pct,
      earned = total_entries >= progress_target,
      earned_at = CASE 
        WHEN total_entries >= progress_target AND user_badges.earned_at IS NULL 
        THEN NOW() 
  ...
  LOOP
    ...
    progress_pct := LEAST(100, ROUND((COALESCE(user_profile.current_streak, 0)::NUMERIC / progress_target::NUMERIC) * 100));
    
    RAISE LOG 'Processing streak badge: %, target: %, current: %, percentage: %', 
      badge_id, progress_target, COALESCE(user_profile.current_streak, 0), progress_pct;
    ...
    VALUES (
      ...
      progress_pct,
      COALESCE(user_profile.current_streak, 0) >= progress_target,
      CASE WHEN COALESCE(user_profile.current_streak, 0) >= progress_target THEN NOW() ELSE NULL END
    )
    ON CONFLICT (user_id, badge_id) 
    DO UPDATE SET 
      progress_current = COALESCE(user_profile.current_streak, 0),
      progress_percentage = progress_pct,
      earned = COALESCE(user_profile.current_streak, 0) >= progress_target,
      earned_at = CASE 
        WHEN COALESCE(user_profile.current_streak, 0) >= progress_target AND user_badges.earned_at IS NULL 
        THEN NOW() 
        ELSE user_badges.earned_at 
      END;
  END LOOP;
...
BEGIN
  ...
  LOOP
    -- Calculate progress percentage
    progress_pct := LEAST(100, ROUND((entry_count::NUMERIC / progress_target::NUMERIC) * 100));
    
    UPDATE user_badges
    SET progress_current = entry_count,
        progress_percentage = progress_pct,
        earned = entry_count >= progress_target,
        earned_at = CASE 
          WHEN entry_count >= progress_target AND earned_at IS NULL THEN NOW() 
          WHEN entry_count < progress_target THEN NULL
          ELSE earned_at 
        END
    WHERE user_id = OLD.user_id AND badge_id = badge_id;
  END LOOP;
...
BEGIN
  IF EXISTS (
    ...
    WHERE tablename = 'user_badges' AND policyname = 'Users and functions can update badges'
  ) THEN
    DROP POLICY "Users and functions can update badges" ON user_badges;
  END IF;
END
$$;
...
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'user_badges' AND policyname = 'Users and functions can insert badges'
  ) THEN
    DROP POLICY "Users and functions can insert badges" ON user_badges;
  END IF;
END
$$;
